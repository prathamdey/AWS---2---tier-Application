#####################################################################################
## Module Desc: Create an ASG, Launch Template, Plocy for Scale up & Down and alarm 
## Date: 08/08/2023
## Ver: v1
#####################################################################################
...
# scale up alarm
# alarm will trigger the ASG policy (scale up policy) based on the metric (CPUUtilization), comparison_operator, threshold
...
resource "aws_cloudwatch_metric_alarm" "scale_up_alarm" {
  ...
  period              = "120"
  statistic           = "Average"
  # A new instance will be created once CPU utilization is higher than 70 %
  threshold           = "70" 
  dimensions = {
...
# scale down alarm
# alarm will trigger the ASG policy (scale down policy) based on the metric (CPUUtilization), comparison_operator, threshold
resource "aws_cloudwatch_metric_alarm" "scale_down_alarm" {
  ...
  period              = "120"
  statistic           = "Average"
  # Instance will scale down when CPU utilization is lower than 5 %
  threshold           = "5" 
  dimensions = {
    "AutoScalingGroupName" = aws_autoscaling_group.asg_name.name
  }
